...
"#,
    ...
    let signed = bridge
        .account()
        .sign_tx(&sender.mnemonic, Network::Preprod, 0, 0, &result.tx_cbor)
        .unwrap();

    let response = client
        .post("https://preprod.koios.rest/api/v1/submittx")
        .header("Content-Type", "application/cbor")
        .body(hex::decode(&signed).unwrap())
        .send()
        .await
        .unwrap();

    let tx_hash = response.text().await.unwrap();
    println!("submitted: {}", tx_hash.trim().replace('"', ""));
}
