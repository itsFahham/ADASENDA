...
async fn main() {
    ...
    thread::spawn(move || {
        ...
        while let Some(job) = job_rx.blocking_recv() {
            match job {
                Job::Send { message, lovelace, reply } => {
                    let result = do_send(&bridge, &sender_mnemonic, &w_sender,
                                         &w_receiver, &message, &lovelace);
    ...
    // 3. start the server
    let state = AppState { sender_addr, receiver_addr, jobs: job_tx };
    let app = Router::new()
...
async fn send(
    ...
    if message.len() > 64 {
        return Err(fail(StatusCode::BAD_REQUEST, "message too long (max 64 bytes)"));
    }
    if ada < 1.0 {
    ...
    state.jobs.send(Job::Send {
        message: message.to_string(),
        lovelace,
        reply: reply_tx,
    }).await.map_err(|_| fail(StatusCode::INTERNAL_SERVER_ERROR, "worker gone"))?;

    match reply_rx.await.map_err(|_| fail(StatusCode::INTERNAL_SERVER_ERROR, "no reply"))? {
...
impl ChainDataProvider for KoiosProvider {
    fn utxos(&self, address: &str) -> ccl::Result<Value> {
        ...
        let mapped: Vec<Value> = raw.iter().map(|u| json!({
            "tx_hash": u["tx_hash"],
    ...
    fn protocol_params(&self) -> ccl::Result<Value> {
        let mut params: Vec<Value> = ureq::get(&format!("{KOIOS}/epoch_params"))
            .call().map_err(koios_err)?
            .into_json().map_err(koios_err)?;
...
fn koios_err<E: std::fmt::Display>(e: E) -> ccl::CclError {
    ccl::CclError { code: -1, message: format!("koios: {e}") }
}

fn do_send(bridge: &Bridge, mnemonic: &str, sender_addr: &str,
           receiver_addr: &str, message: &str, lovelace: &str) -> Result<String, String> {
    let yaml = build_yaml(sender_addr, receiver_addr, message, lovelace);
    let provider = KoiosProvider;

    let built = bridge.quicktx()
...
fn submit(signed_hex: &str) -> Result<String, String> {
    ...
    match ureq::post(&format!("{KOIOS}/submittx"))
        ...
        .send_bytes(&bytes)
    {
        Ok(r) => Ok(r.into_string().unwrap_or_default().trim().trim_matches('"').to_string()),
...
async fn history() -> Json<Value> {
    let text = std::fs::read_to_string("../../data/history.json")
        .unwrap_or_else(|_| "[]".to_string());
    let mut list: Vec<Value> = serde_json::from_str(&text).unwrap_or_default();

    let hashes: Vec<String> = list.iter()
...
fn confirmations(tx_hashes: &[String]) -> HashMap<String, i64> {
    ...
    for item in &arr {
        if let Some(h) = item["tx_hash"].as_str() {
            out.insert(h.to_string(), item["num_confirmations"].as_i64().unwrap_or(0));
        }
    }
    out
}
